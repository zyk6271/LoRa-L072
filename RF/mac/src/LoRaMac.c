...
typedef struct sLoRaMacCtx
{
    ...
    */
    rt_timer_t TxDelayedTimer;
    /*
    * LoRaMac reception windows timers
    */
    rt_timer_t RxWindowTimer1;
    rt_timer_t RxWindowTimer2;
    /*
 ...
 */
static void RxWindowSetup( rt_timer_t rxTimer, RxConfigParams_t* rxConfig );
...
static void ProcessRadioTxDone( void )
{
    ...
    rt_timer_control(MacCtx.RxWindowTimer2, RT_TIMER_CTRL_SET_TIME, (void*)&MacCtx.RxWindow2Delay);
    //TimerStart( &MacCtx.RxWindowTimer2 );
    rt_timer_start( MacCtx.RxWindowTimer2 );

    if( ( MacCtx.NvmCtx->DeviceClass == CLASS_C ) || ( MacCtx.NodeAckRequested == true ) )
    {
        getPhy.Attribute = PHY_ACK_TIMEOUT;
        phyParam = RegionGetPhyParam( MacCtx.NvmCtx->Region, &getPhy );
        uint32_t time= MacCtx.RxWindow2Delay + phyParam.Value;
        rt_timer_control( MacCtx.AckTimeoutTimer,RT_TIMER_CTRL_SET_TIME, (void*)&time );
                #if defined( ModeA )
                    TimerSetValue( &MacCtx.AckTimeoutTimer, 0 );
...
static void ProcessRadioRxDone( void )
{
    ...
    rt_timer_stop( MacCtx.RxWindowTimer2 );
...
static void LoRaMacHandleMcpsRequest( void )
{
    ...
    if( MacCtx.MacFlags.Bits.McpsReq == 1 )
    {
        ...
        {// Stop retransmission
            ...
            rt_timer_stop( MacCtx.TxDelayedTimer );
...
static void OnAckTimeoutTimerEvent( void* context )
{
    rt_timer_stop( MacCtx.AckTimeoutTimer );
...
//                TimerStart( &MacCtx.TxDelayedTimer );
                rt_timer_control( MacCtx.TxDelayedTimer,RT_TIMER_CTRL_SET_TIME, (void*)&dutyCycleTimeOff );
                rt_timer_start( MacCtx.TxDelayedTimer );
            }
            return LORAMAC_STATUS_OK;
        }
        else
...
void OpenContinuousRxCWindow( void )
{
...
LoRaMacStatus_t LoRaMacInitialization( LoRaMacPrimitives_t* primitives, LoRaMacCallback_t* callbacks, LoRaMacRegion_t region )
{
    ...
    // Initialize timers
    MacCtx.TxDelayedTimer = rt_timer_create("MacCtx.TxDelayedTimer", OnTxDelayedTimerEvent,RT_NULL, 10,RT_TIMER_FLAG_PERIODIC);
    MacCtx.RxWindowTimer1 = rt_timer_create("MacCtx.RxWindowTimer1", OnRxWindow1TimerEvent,RT_NULL, 10,RT_TIMER_FLAG_PERIODIC);
    MacCtx.RxWindowTimer2 = rt_timer_create("MacCtx.RxWindowTimer2", OnRxWindow2TimerEvent,RT_NULL, 10,RT_TIMER_FLAG_PERIODIC);
    MacCtx.AckTimeoutTimer = rt_timer_create("MacCtx.AckTimeoutTimer", OnAckTimeoutTimerEvent,RT_NULL, 10,RT_TIMER_FLAG_PERIODIC);
