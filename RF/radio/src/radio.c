...
void RadioInit( RadioEvents_t *events )
{
   ...
   // TimerInit( &TxTimeoutTimer, RadioOnTxTimeoutIrq );
    TxTimeoutTimer = rt_timer_create("TxTimeoutTimer", RadioOnTxTimeoutIrq,RT_NULL, 10,RT_TIMER_FLAG_PERIODIC);
    //TimerInit( &RxTimeoutTimer, RadioOnRxTimeoutIrq );
    RxTimeoutTimer = rt_timer_create("RxTimeoutTimer", RadioOnRxTimeoutIrq,RT_NULL, 10,RT_TIMER_FLAG_PERIODIC);
                        ...
                        uint8_t hopPeriod, bool iqInverted, uint32_t timeout )
{
    ...
    // WORKAROUND - Modulation Quality with 500 kHz  Bandwidth, see DS_SX1261-2_V1.2 datasheet chapter 15.1
...
void RadioSend( uint8_t *buffer, uint8_t size )
{
    ...
    rt_timer_control( TxTimeoutTimer, RT_TIMER_CTRL_SET_TIME,(void*)&TxTimeout );
    rt_timer_start( TxTimeoutTimer );
}
...
void RadioRx( uint32_t timeout )
{
    ...
    if( timeout != 0 )
    {
        rt_timer_control( RxTimeoutTimer,RT_TIMER_CTRL_SET_TIME, (void*)&timeout );
        rt_timer_start( RxTimeoutTimer );
    }
...
void RadioSetTxContinuousWave( uint32_t freq, int8_t power, uint16_t time )
{
    ...
    rt_timer_control( TxTimeoutTimer,RT_TIMER_CTRL_SET_TIME, (void*)&time);
    rt_timer_start( TxTimeoutTimer );
}
